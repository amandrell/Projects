...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        x = operands.map(lambda y: scheme_eval(y, env))
        return scheme_apply(self, x, env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of parameters were passed into {0}' .format(self))
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        x = scheme_apply(self, operands, env)
        y = complete_eval(x)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        next_expression = expressions.second.first
        env.define(target, scheme_eval(next_expression, env))
        return target
        # END PROBLEM 6
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(bind, 2, 2)
        s = Pair(bind.first, s)
        value = scheme_eval(bind.second.first, env)
        vals = Pair(value, vals)
        bindings = bindings.second


    check_formals(s)
    return env.make_child_frame(s, vals)
